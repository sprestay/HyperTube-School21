...
export default function Library () {
  ...
  async function displayData (str, payload) {
    ...
    const apiUri = (() => {
      switch (str) {
        case 'popular':
          return `/library?page=${payload || 1}`
        case 'lastadded':
          return `/library/lastadded?page=${payload || 1}`
        case 'random':
          return '/library/random'
        case 'search':
          return `/library/search?name=${payload || '42'}`
        default:
          return `/library?page=${payload || 1}`
      }
    })()
  ...
  if (!state.login)
    ...
    return (
      <>
        ...
        <SortBar state={state} movies={movies} tmp={setTmp} filters={filters}
                 setFilters={setFilters} sort={sort} setSort={setSort}/>
